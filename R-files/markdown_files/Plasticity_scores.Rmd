...
---
title: "Revised Grouping of Plasticity Scores"
output: html_document
---

## Revised Grouping of Plasticity Scores

### Group 1: Dispersion and Variability Measures
*These indices capture the overall spread or variability of trait values across environments.*

- **CV_t:** Coefficient of Variation (SD/mean)
- **CEV:** Coefficient of Environmental Variation
- **PR:** Phenotypic Range (max - min)
- **NRW:** Norm of Reaction Width (range-based measure)
- **SI:** Stability Index  
  \(\displaystyle SI = \frac{\mathrm{Var}(\{\bar{T}_e\})}{\mathrm{Mean}(\{\bar{T}_e\})}\)
- **RSI:** Relative Stability Index  
  \(\displaystyle RSI = 1 - \frac{SD(\{\bar{T}_e\})}{\mathrm{Mean}(\{\bar{T}_e\})}\)
- **EVS:** Environmental Variance Sensitivity  
  \(\displaystyle EVS = \frac{\mathrm{Var}(T)}{\mathrm{Var}(E)}\)

---

### Group 2: Slope/Gradient and Change Measures
*These indices quantify the rate or slope of trait change along environmental gradients.*

- **RN:** Reaction Norm Slope (linear regression coefficient)
- **RNN:** Nonlinear Reaction Norm (captures nonlinearity in the response)
- **D_slope:** D Slope (difference between upper and lower fractions)
- **RC:** Response Coefficient (ratio of mean trait values in contrasting environments)
- **RTR:** Relative Trait Response (scaled difference between trait means)
- **PSI:** Phenotypic Stability Index  
  \(\displaystyle PSI = \frac{1}{1 + |\beta|}\)  
  *(with \(\beta\) as the regression slope)*
- **RPI:** Relative Plasticity Index (average of pairwise relative differences)
- **PQ:** Plasticity Quotient (change in trait divided by change in environment)  
  \(\displaystyle PQ = \frac{\Delta \text{Trait}}{\Delta \text{Environment}}\)
- **APC:** Absolute Plasticity Coefficient (mean absolute change between consecutive environments)

---

### Group 3: Relative and Adjusted Means Measures
*These indices compare environment-specific summaries or adjusted means to capture plasticity.*

- **gPi:** Grand Plasticity Index (SD of adjusted means divided by their mean)
- **PPF:** Phenotypic Plasticity Factor (percentage difference between two least-square means)
- **PPi:** Phenotypic Plasticity Index  
  \(\displaystyle PPi = \frac{T_{\max} - T_{\min}}{T_{\max}}\)
- **PImd:** Proportional Inter-Median Difference  
  \(\displaystyle PImd = \frac{\max(\{\tilde{T}_e\}) - \min(\{\tilde{T}_e\})}{\max(\{\tilde{T}_e\})}\)
- **PILSM:** Proportional Inter-Least Square Mean Difference  
  \(\displaystyle PILSM = \frac{\max(\{LSM_e\}) - \min(\{LSM_e\})}{\max(\{LSM_e\})}\)
- **PIR:** Phenotypic Instability Ratio (difference between max and min environment means, relative to a reference value)
- **RDPI:** Relative Distance Plasticity Index  
  \(\displaystyle RDPI = \frac{1}{\binom{n}{2}} \sum_{i<j} \frac{|T_i - T_j|}{T_i + T_j}\)
- **ESPI:** Environmental Sensitivity Performance Index  
  \(\displaystyle ESPI = \frac{\text{Max Environment Mean} - \text{Min Environment Mean}}{|\max(E) - \min(E)|}\)
- **ESPIID:** Environmental Sensitivity Performance Index for Individual Differences  
  \(\displaystyle ESPIID = \frac{\text{Mean (or Median) Absolute Phenotypic Distance}}{|\Delta E|}\)
- **ESP:** Environment-Specific Plasticity (deviation from the overall mean)
- **PFI:** Phenotypic Flexibility Index (maximum deviation from baseline relative to the baseline)
